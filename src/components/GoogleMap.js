...
export default class GoogleMap extends Component {
    ...
    const markersArray = markers.map(marker => {
      ...
      if (marker.message) {
        ...
        google.maps.event.addListener(thisMarker, "click", () =>
          this.handleMarkerClick(thisMarker, marker.message)
        );
      }
      return thisMarker;
    });
  ...
  render() {
    let url = "http://maps.googleapis.com/maps/api/js?key=" + MAPS_API_KEY;
    return (
      <div className="GMap">
        <Script
          url={url}
